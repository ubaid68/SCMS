	...
	public function accessRules()
	{
		...
		*/	
			...
			array('deny',  // deny all users
				'roles'=>array('salesMan','materialMaterial','productManager'),
			)
		/*	
		...
		if($_POST['Employee']['role']==null)
			...
			if($model->save())
				if($model->role=='manager')
				{
                    		...
                    		$auth->assign('manager', $model->login_id);
                    		$auth->save();
				//$this->redirect(array('view','id'=>$model->login_id));
				Yii::app()->user->setFlash('empsuccess','Employee  Added Successfully');
							$this->refresh();
				}
				if($model->role=='productManager')
				{
                    		$auth = Yii::app()->authManager;
                    		$auth->assign('productManager', $model->login_id);
                    		$auth->save();
				//$this->redirect(array('view','id'=>$model->login_id));
				Yii::app()->user->setFlash('empsuccess','Employee  Added Successfully');
							$this->refresh();
				}
				if($model->role=='materialManager')
				{
                    		$auth = Yii::app()->authManager;
                    		$auth->assign('materialManager', $model->login_id);
                    		$auth->save();
				//$this->redirect(array('view','id'=>$model->login_id));
				Yii::app()->user->setFlash('empsuccess','Employee  Added Successfully');
							$this->refresh();
				}
				if($model->role=='salesMan')
				{
                    		$auth = Yii::app()->authManager;
                    		$auth->assign('salesMan', $model->login_id);
                    		$auth->save();
				//$this->redirect(array('view','id'=>$model->login_id));
				Yii::app()->user->setFlash('empsuccess','Employee  Added Successfully');
							$this->refresh();
